...
- **Brightness everywhere**: controls the real backlight of external monitors (DDC), dims via software on monitors that don't support that, and can keep dimming below the hardware minimum. Smooth fades, and brightness keys that follow the pointer, target all displays, or a chosen subset. Enable **Fine Brightness Steps** in Settings for six smaller steps per normal brightness-key step, or hold Option while pressing a brightness key
