...
- **Glass only on the backdrop.** The panel uses `NSGlassEffectView` (regular).
  ...
  full-width shaded band (`NSColor.labelColor` at 8% opacity, painted on the
  block's clip layer by `PanelCanvas.tintBands()`), not a floating rounded card.
  *Materials HIG: content layer uses standard materials, no glass-on-glass.*
  The OSD banner (OSDBannerService) is the one surface outside the panel that samples what is behind it. On macOS 26 it is not glass: no glass material can hold the system HUD's backdrop, which is softened far less than any of them soften it, so the banner blurs its own backdrop under a flat grey. On macOS 27 the system offers a glass variant that can hold that backdrop, and the banner draws the capsule with it (`OSDBannerService.makeGlassView`); the panel is no longer the only glass in the app there.
